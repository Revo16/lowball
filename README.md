...
- **A leg you pick yourself is yours:** only you can change or remove it (on Find a bet), until the lock; the admin can still fix it. **A leg someone entered for you** (texted-in picks, via **Enter for them**) is tagged "Entered by …" and anyone can change it or remove it until the parlay is placed; the owner gets a push either way. Re-pick it yourself and it's locked to you. On the Bookie tab every leg is its own card, Gmail-style: a neon-yellow left edge means swipe right to change (or tap **Change**), a red right edge means swipe left to remove (with Undo). Remove works on legs entered for someone; legs a player picked himself show a lock. The admin gets both swipes on every leg.
