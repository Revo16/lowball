...
- You can change your own leg until the lock. Anyone can enter a leg for a pool member who has none (for picks texted in) via **Enter for them** on the Bookie tab, until it's placed: a teal "Picking for …" bar shows the mode, the owner gets a push notification, the leg is tagged "Entered by …", and you land back on the Bookie tab. Only the admin can change someone else's existing leg.
